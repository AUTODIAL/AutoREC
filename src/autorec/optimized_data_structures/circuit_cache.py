...
Provides efficient caching mechanisms to avoid re-computing expensive circuit 
parameter fitting operations. Includes both Clock (Second-Chance) and LRU cache 
implementations with configurable size limits.
"""
...
class ClockCache:
    ...
    Uses a circular buffer with a "reference bit" to determine eviction.
    Items get a second chance before eviction, making it good for workloads
    with temporal locality (recently used items are likely to be used again).
...
class HistoryCache:
    ...
    This class provides a simple interface for caching expensive circuit
    parameter fitting operations. It handles the creation of cache keys
    ...
    def __init__(self, capacity: int = 20000, cache_type: str = 'lru'):
        ...
        Args:
            ...
            cache_type: Eviction policy to use. Supported values are ``"lru"``
                and ``"clock"``.
        """
        if cache_type == 'lru':
    ...
    def _make_key(self, circuit_code: str, eis_index: int, 
                  action_type: str, action_position: int) -> Tuple:
        """
    ...
    def get(self, circuit_code: str, eis_index: int,
            action_type: str, action_position: int) -> Optional[Dict[str, Any]]:
        """
