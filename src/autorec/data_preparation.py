...
class EISDataPrep:
    ...
    @staticmethod
    def flatten_EIS(Z_norm, angles_norm, mag_norm, mag_scaled):
        """
        ...
        # Z_stacked = np.dstack((real, imag))  # real1, imag1, real2, imag2, ...
        # Z_stacked = np.stack((real, imag))   # real1, real2, ..., imag1, imag2, ..
        # Z_stacked = np.stack((imag, angles_norm))
        # Z_stacked = np.stack((angles_norm, mag_norm))
        # Z_stacked = np.stack((angles_norm, mag_scaled))
        Z_stacked = np.stack((imag, angles_norm, mag_scaled, -angles_norm))
        # Z_stacked = np.stack((imag, angles_norm, mag_scaled, -imag, -angles_norm, -mag_scaled))

        flatten_Z = Z_stacked.flatten()
        return flatten_Z
    ...
    def load_single_csv(self, csv_path: Path, base_path: Path) -> Optional[dict]:
        ...
        try:
            ...
            Z_norm, angles_norm, mag_norm, mag_scaled = self.normalize_EIS(Z_true)
            flatten_Z = self.flatten_EIS(Z_norm, angles_norm, mag_norm, mag_scaled)
